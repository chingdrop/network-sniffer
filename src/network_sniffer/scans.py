...
from network_sniffer.packet import BroadcastAdapter


bca = BroadcastAdapter()


def ack_scan(target: str, ports: list[int], verbose=True) -> Tuple[list, list]:
    src_port = RandShort()
    unfiltered_ports = []
    filtered_ports = []
    closed_ports = []
    ...
    for s, r in ans:
        if r.haslayer(TCP) and r[TCP].flags == TcpFlags.RST_PSH:
            ...
            r.haslayer(ICMP)
            and r[ICMP].type == ICMP_DESTINATION_UNREACHABLE
            and r[ICMP].code in self.icmp_codes
        ):
...
def xmas_scan(target: str, ports: list[int], verbose=True) -> Tuple[list, list]:
    src_port = RandShort()
    open_ports = []
    filtered_ports = []
    closed_ports = []
...
def protocol_scan(self, target: str, protos: list[int], verbose=True) -> list[int]:
